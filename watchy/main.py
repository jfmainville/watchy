...
from tmdb import (
    tmdb_authenticate,
    tmdb_remove_session,
    tmdb_extract_watchlist,
    tmdb_extract_movie_imdb_id,
    tmdb_extract_movie_release_dates,
    tmdb_extract_show_details,
...
content_cleanup_days = int(os.environ.get("CONTENT_CLEANUP_DAYS"))
...
parser.add_argument(
    ...
    dest="movie",
    action="store_true",
    help="execute the movie related function",
)
parser.add_argument(
    "--tv",
    "-t",
    dest="tv",
    action="store_true",
    help="execute the TV show related function",
)

args = parser.parse_args()

# Extract all the movies from the TMDB API
tmdb_session_id = tmdb_authenticate(
...
def movie():
    ...
    tmdb_watchlist_movies = tmdb_extract_watchlist(
        ...
        tmdb_api_key=tmdb_api_key,
        tmdb_watchlist_content_type="movies",
    )
    ...
    if tmdb_watchlist_movies:
        for tmdb_watchlist_movie in tmdb_watchlist_movies:
            tmdb_movie_release_dates = tmdb_extract_movie_release_dates(
                tmdb_api_url=tmdb_api_url,
                tmdb_api_key=tmdb_api_key,
            ...
            tmdb_movie_imdb_id = tmdb_extract_movie_imdb_id(
                tmdb_api_url=tmdb_api_url,
                tmdb_api_key=tmdb_api_key,
                tmdb_watchlist_movie=tmdb_watchlist_movie,
            )
            ...
            if tmdb_movie_release_dates and tmdb_movie_imdb_id:
                ...
                tmdb_movie_release_year = (tmdb_watchlist_movie["release_date"]).split(
                    "-"
                )[0]
                ...
                if tmdb_movie_dvd_release_date_convert < today:
                    ...
                    seeds, magnet_link = yts_extract_movie_torrent(
                        movie_imdb_id=tmdb_movie_imdb_id,
                    ...
                    if seeds and magnet_link:
                        ...
                        download_movie = {}
                        tmdb_movie_title_full = (
                            (tmdb_movie_title.replace(":", " -"))
                            + " ("
                        ...
                        if download_movie != {}:
                            ...
                            return_code = download_magnet_link(
                                download_entry=download_movie,
...
def tv_show():
    ...
    tmdb_shows = tmdb_extract_watchlist(
        ...
        tmdb_session_id=tmdb_session_id,
        tmdb_api_key=tmdb_api_key,
        tmdb_watchlist_content_type="tv",
    )

    if tmdb_shows:
        for tmdb_show in tmdb_shows:
            tmdb_show_details = tmdb_extract_show_details(
                tmdb_api_url=tmdb_api_url,
                tmdb_api_key=tmdb_api_key,
                tmdb_show=tmdb_show,
            )
            ...
            if tmdb_show_details:
                tmdb_show_imdb_id = (
                    tmdb_show_details["external_ids"]["imdb_id"]
                ).replace("tt", "")
                tmdb_show_name = re.sub(
                    "[^a-zA-Z0-9 \n]", "", unidecode.unidecode(tmdb_show["name"])
                )
                # Create the local TV show directory if it doesn't already exist
                create_content_folders(
                ...
                eztv_shows = eztv_extract_tv_show_episodes(
                    imdb_id=tmdb_show_imdb_id, eztv_url=eztv_url
                )
                eztv_show_listdict = []
                if eztv_shows:
                    for eztv_show in eztv_shows:
                        eztv_show_episode = (
                            "S"
                            + str(eztv_show["season"]).zfill(2)
                            + "E"
                            + str(eztv_show["episode"]).zfill(2)
                        )
                        eztv_show_title = (eztv_show["title"]).split(eztv_show_episode)[
                            0
                        ]
                        ...
                        eztv_show_full_name = (
                            tmdb_show_name.title() + " " + eztv_show_episode
                        )
                ...
                for filtered_eztv_show_dictionary_item in filtered_eztv_show_listdict:
                    eztv_show_title_filtered = filtered_eztv_show_dictionary_item[
                        "name"
                    ]
                ...
                for download_tv_show in download_tv_shows:
                    ...
                    return_code = download_magnet_link(
                        download_entry=download_tv_show,
                    ...
                    move_content_file(
                        download_file=download_tv_show,
                        content_download_folder=tv_shows_download_directory,
                        content_folder=tv_shows_directory,
                        content_title=tmdb_show_name,
                        return_code=return_code,
